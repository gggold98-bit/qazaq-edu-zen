...
export function AuthGate() {
  const { login, loginWithGoogle, register } = useAppStore();
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (!r.ok) toast.error(r.error || "Кіру сәтсіз");
    else toast.success("Қош келдіңіз!");
  };
  const handleRegister = (e: React.FormEvent) => {
    e.preventDefault();
    const r = register(email, password, name);
    if (!r.ok) toast.error(r.error || "Тіркелу сәтсіз");
    else toast.success("Тіркелу сәтті аяқталды!");
  };
        ...
        >
          ...
          <div className="space-y-6">
            <h1 className="text-5xl font-semibold leading-tight tracking-tight">
              Ұстаздың{" "}
              <span className="text-gradient">құқығы</span> мен
              <br />
              әдістемесі бір жерде.
            </h1>
            ...
            <div className="flex gap-3">
              {["514-бұйрық", "Заң қорғауы", "AI көмекші"].map((t) => (
                <span key={t} className="glass rounded-full px-4 py-1.5 text-xs font-medium">
                  {t}
                </span>
              ))}
            </div>
          ...
          <div className="text-xs text-muted-foreground">
            © 2026 Qazaq Teachers AI. Барлық құқықтар қорғалған.
          </div>
        </motion.div>
        ...
        >
          <div className="glass w-full max-w-md rounded-3xl p-8 shadow-2xl">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Аккаунтыңызға кіріңіз немесе жаңадан тіркеліңіз
            </p>
            ...
            <Tabs value={tab} onValueChange={(v) => setTab(v as "login" | "register")} className="mt-6">
              <TabsList className="grid w-full grid-cols-2 rounded-xl bg-muted/60 p-1">
                ...
                <TabsTrigger value="register" className="rounded-lg data-[state=active]:bg-card data-[state=active]:shadow-sm">
                  Жаңадан тіркелу
                </TabsTrigger>
              </TabsList>
              ...
              <TabsContent value="login" className="mt-6">
                <form onSubmit={handleLogin} className="space-y-4">
                  ...
                  <Button type="submit" className="h-11 w-full gradient-emerald font-semibold shadow-lg shadow-primary/30">
                    Кіру
                  </Button>
                </form>
              </TabsContent>
            ...
            <Button
              type="button"
              variant="outline"
              onClick={() => { loginWithGoogle(); toast.success("Google арқылы кірдіңіз"); }}
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="relative">
        ...
        <Input id="email" type="email" autoComplete="email" placeholder="you@gmail.com" value={email} onChange={(e) => setEmail(e.target.value)} className="h-11 pl-10" />
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="relative">
        ...
        <Input id="password" type={showPass ? "text" : "password"} placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} className="h-11 px-10" />
